            ...
            <Container fluid style={{'overflow':'auto','height':'85vh'}}>
                {
                    this.props.listings && this.props.listings.length > 0? this.props.listings.map(listing => 
                        <Card key={listing._id} image={listing.suite.building.photos.length == 0? 'https://www.jeevesfloridarentals.com/images/default_house.jpg': `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photo_reference=${listing.suite.building.photos[0]}&key=${process.env.REACT_APP_API_KEY}`}
